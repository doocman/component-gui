...
#include <asp/asp.hpp>
#include <asp/dynamic.hpp>
#include <asp/embedded/asp_example_font.hpp>
#include <asp/ft_fonts.hpp>
#include <asp/sdl.hpp>

constexpr auto gen_radio_buttons(auto &cached_font) {
  return asp::widget_builder()
      .event(
          asp::radio_button_trigger()
              .elements(asp::dynamic::uni_sized_widget_list_builder().displays(
                  asp::display_per_state(asp::fill_rect()),
...
constexpr auto gen_viewed_widget(auto &&w) {
  static_assert(asp::view_port_trigger::sub_widget<decltype(w)>);
  return asp::widget_builder()
      .event(asp::view_port_trigger::builder()
  ...
  try {
    ...
    auto gui =
        asp::gui_context_builder()
            .on_resize([](auto &&sz, auto &&ws) {
              ...
              auto full_area = asp::box_from_xywh<asp::default_point_rect>(
                  0, 0, asp::call::width(sz), asp::call::height(sz));
              auto button_area = asp::trim_from_below(
                  &full_area, std::min(asp::point_unit(120),
                                       asp::call::height(full_area)));
              end_button.area(button_area);
              asp::call::area(w, full_area);
            })
                     ...
                     asp::widget_builder()
                         .event(asp::buttonlike_trigger(
                             asp::momentary_button()
                                 ...
                                 .build()))
                         .display(asp::text_renderer(std::ref(cached_font)))
                         .build())
            .build(full_area);
    ...
    list.access_trigger([&](auto &&viewed) {
      viewed.access_trigger([argc, argv, &end_button,
        ...
        auto display_creator = [&prototype, &f_prototype, &end_button,
          ...
          auto f = f_prototype;
          f.get(asp::radio_button::trigger_on{}) =
    ...
    while (!do_exit) {
      ...
      if (rerender_all) {
        ...
        renderer.render_to(gui);
        renderer.present();
      } else if (!asp::empty_box(to_rerender)) {
        renderer.clear();
