...
public:
  ...
  ft_font_glyph(FT_Glyph g, FT_Int top) : g_(g), top_(top) {
    ...
    ASP_ASSERT(bitmap_gl->format == FT_GLYPH_FORMAT_BITMAP);
  }
#else
  ...
  constexpr expected<void, FT_Error> render(renderer auto &&rend) const {
    ...
    ASP_ASSERT(bitmap.pitch >= 0);
    ASP_ASSERT(static_cast<unsigned int>(bitmap.pitch) <= bitmap.width);
    call::draw_alpha(
        rend,
        ...
        [&](pixel_unit_t<default_rect> const &b, auto &&px_rend) {
          for (auto y : asp::y_view(b.value())) {
            for (auto x : asp::x_view(b.value())) {
    ...
    call::draw_alpha(
        ...
        [&](bounding_box auto const &b, auto &&px_rend) {
          for (auto y : asp::y_view(b)) {
            for (auto x : asp::x_view(b)) {
              px_rend(
...
public:
  ...
  [[nodiscard]] constexpr pixel_unit_t<int> full_height() const noexcept {
    return pixel_unit_t<int>(handle()->size->metrics.height >> 6);
  }

  auto ascender() const { return pixel_unit_t<int>(handle()->ascender >> 6); }
};
