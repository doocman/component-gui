...
template <font_face TFont> class text_renderer {
  ...
  constexpr void assert_displayed() const {
      ...
      std::visit(
          [&]<typename T>(T &t) {
            if constexpr (std::is_same_v<T, glyph_entry>) {
              ...
              ASP_ASSERT(cur_len >= measured_len);
            } else {
    ...
    ASP_ASSERT(measured_lines == line_count_);
  }
...
public:
  ...
  {
    ASP_DEBUG_ONLY(bool _area_initialised{};)
    if (empty(text_)) {
      return;
    }
    ...
    for (auto const &t : tokens_) {
      std::visit(
          [r = rorg.with(colour_), &area,
           &do_new_area ASP_DEBUG_ONLY(, &_area_initialised)]<typename T>(
              T const &tok) {
            if constexpr (std::is_same_v<T, glyph_entry>) {
              ...
              ASP_DEBUG_ONLY(_area_initialised = true;)
            }
          },
          t);
    }
...
public:
  ...
  constexpr void handle(A const &, Evt &&evt, BP &&back_prop) {
    ...
    if constexpr (interpreted_event_types<Evt,
      ...
      //                                      [this, &back_prop, &evt]<typename
      //                                      Sub>(
      //                                          Sub &&s, element_id_t index) {
      //                                        this->intr_event_switch(back_prop,
      //                                        s,
      //                                                                index)(
      //                                            std::forward<Evt>(evt));
      //                                      })) {
  ...
  constexpr auto accessor(point_rect auto const &r, access_each_element_t tag)
    requires(requires() {
      { elements().accessor(r, tag) } -> asp::accessor;
    })
  {
