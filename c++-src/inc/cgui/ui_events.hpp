...
template <input_events... evts> struct subset_input_events {
  ...
  input_events val;
  constexpr explicit(false) subset_input_events(input_events v) noexcept : val(v) {
    CGUI_ASSERT(((v == evts) || ...));
  }
  constexpr explicit(false) operator input_events() const noexcept { return val; }
...
template <input_events evt> struct subset_input_events<evt> {
  ...
  constexpr explicit(false) operator input_events() const noexcept { return evt; }
...
template <typename T>
concept subset_input_event_c = std::convertible_to<T, input_events> && requires() {
  {
